...
interface NewsListClientProps {
  newsItems: NewsAndStoriesDetailsDocument[];
  showFilters?: boolean;
}

const NewsListClient: React.FC<NewsListClientProps> = ({
  newsItems,
  showFilters = true,
}) => {
  ...
  const categories: { id: Category; label: string }[] = [
    ...
    { id: "Events and Campaigns", label: "Events and Campaigns" },
  ...
  const filteredAndSortedNews = useMemo(() => {
    ...
    if (selectedCategory !== "all") {
      filtered = newsItems.filter((item) => {
        const itemCategory = item.data.categories;
    ...
    switch (sortBy) {
      case "title_asc":
        sorted.sort((a, b) => {
          const titleA = getRichTextAsString(a.data.title);
          const titleB = getRichTextAsString(b.data.title);
    ...
    return sorted;
  }, [newsItems, selectedCategory, sortBy]);

  // Group items into carousel slides
  const { displayNews, slides, totalSlides, maxIndex } = useMemo(() => {
    const itemsPerView = 4; // Show 4 items at a time
    const news = filteredAndSortedNews;

    // Group items into slides
    const slideGroups: NewsAndStoriesDetailsDocument[][] = [];
    for (let i = 0; i < news.length; i += itemsPerView) {
      slideGroups.push(news.slice(i, i + itemsPerView));
    }
                        ...
                        <div className="mt-4 flex flex-col gap-3 flex-1">
                          ...
                          {item.data.description && (
                            <p className="text-sm text-wca-gray line-clamp-3">
                              {getRichTextAsString(item.data.description)}
                            </p>
                          )}
